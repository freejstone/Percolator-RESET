...
train_power, std_power, true_power, df_new, train_decoys_new = do_svm(df_new_temp, train_decoys, folds=3, Cs=[
            0.1, 1, 10], p = p, total_iter=10, kernel='poly', alpha=FDR_threshold, train_alpha=0.01, degree=2, remove=remove, top_positive=top_positive)
...
df_nah_temp = df_new_temp[~(q_val <= 0.15)].copy()

df_new_temp = df_new_temp[(q_val <= 0.15)]
