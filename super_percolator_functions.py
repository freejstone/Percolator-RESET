...
def peptide_level(df_all, peptide_list_df, precursor_bin_width = 1.0005079/4):
    ...
    if 'TailorScore' in df_all.columns:
        df_all = df_all.sort_values(by = 'TailorScore', ascending = False) #sort by score
    else:
        df_all = df_all.sort_values(by = 'XCorr', ascending = False) #sort by score
    ...
    if 'TailorScore' in df_all.columns:
        df_all = df_all.assign(max_tailor_score = df_all.groupby('original_target').TailorScore.transform('max'))
    ...
    if 'XCorr' in df_all.columns:
        df_all = df_all.assign(max_xcorr_score = df_all.groupby('original_target').XCorr.transform('max'))
