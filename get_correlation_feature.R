...
get_correlation_feature = function(file, p, file_path) {
  df = read_delim(file)
  df$corr_feature = 0
  df$corr_feature[df$Label == 1] = sample(c(0, 1), size = sum(df$Label == 1), replace = TRUE, prob = c(0.5 - 0.5*p, 0.5 + 0.5*p))
  df$corr_feature[df$Label == -1] = sample(c(1, 0), size = sum(df$Label == -1), replace = TRUE, prob = c(0.5 - 0.5*p, 0.5 + 0.5*p))
