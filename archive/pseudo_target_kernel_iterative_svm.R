...
radial_basis = function(x, y) {
  diff = t(apply(y, 1, function(y){x - y}))
  return(exp(-rowSums(diff^2)/length(x)))
}
...
Charge2 = SVM_train_features$Charge2
...
Cs_pos = Cs_neg = c(0.0001, 0.001, 0.01, 0.1)
...
for (iter in 1:total_iter) {
  ...
  Cs = data.frame(Cs_pos = double(), Cs_neg = double())
  ...
  for (i in 1:length(Cs_neg)) {
    ...
    for (j in 1:length(Cs_pos)) {
      ...
      model = svm(factor(SVM_labels_iter) ~.,
  ...
  C_max = c(Cs[power_indx, 1,], Cs[power_indx, 2])
  names(C_max) = c(1,-1)
  ...
  combined_file_features = combined_file %>% select(ExpMass, CalcMass, deltCn, XCorr, TailorScore,
                                                    PepLen, Charge2, Charge3, lnNumSP, dM, absdM)
  Charge2 = combined_file_features$Charge2
  combined_file_features = combined_file_features %>% select(-Charge2, -Charge3) %>% scale() %>% as.data.frame()
  combined_file_features = cbind(combined_file_features, factor(Charge2))

  new_scores = predict(model, combined_file_features, decision.values = TRUE)
  new_scores = as.vector(attr(new_scores, 'decision.values'))
