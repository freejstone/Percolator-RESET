...
SVM_train_features = SVM_train_data %>% select(pseudolabel, ExpMass, CalcMass, deltCn, XCorr, TailorScore,
...
x = model.matrix(pseudolabel~., SVM_train_features)[,-1]
y = ifelse(SVM_train_features$pseudolabel > 0, 1, 0)

# print('yes')
# log_labels = SVM_labels
# log_labels[log_labels == -1] = 0
...
x.test = model.matrix(Label~., combined_file_features)[, -1]
Label = combined_file$Label
new_scores = model %>% predict(x.test)
Label = Label[order(-new_scores)]

q_val = TDC_flex_c(Label == -1, Label == 1)
sum(q_val <= 0.01 & Label == 1) #6183
